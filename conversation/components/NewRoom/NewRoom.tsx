...
export default function NewRoom(props: {
  onNewRoom: (params: { roomName: string; usersInvited?: string }) => void;
  onClose: () => void;
}) {
  const [roomName, setRoomName] = useState(`room name ${Date.now()}`);
  const [usersInvited, setUsersInvited] = useState<undefined | string>(
    undefined
  );
  ...
  return (
    <>
      <div className="my-4 p-4">
        <p className="py-2">New Room</p>

        <form
          onSubmit={(e) => {
            e.preventDefault();
            props.onNewRoom({ roomName, usersInvited });
          }}
        >
          <div className="rounded-md shadow-sm space-y-px">
            ...
            <div className="relative">
              <input
                ...
                value={usersInvited}
                onChange={(e) => setUsersInvited(e.target.value)}
              />
            </div>
          </div>
